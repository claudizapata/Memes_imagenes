...
const imgDown =(e)=>{//esto es para exportar archivos
        html2canvas(document.getElementById('exportar'), {allowTaint:true,useCORS:true}).then(function(canvas){
        ...
        <div>
            ...
            <h2 className='mt-2 mb-3 titulo2' >Elegí la imagen de tu meme</h2>           
             <select onChange={seleccionarImg} className="" aria-label="Default select example">
                ...
                <option  value={`gallito1`}>Gallito</option>
                <option  value={`hombre-araña1`}>Hombre Araña</option>
                <option  value={`jirafa1`}>Jirafa</option>
                <option  value={`palmera1`}>Palmera</option>
                <option  value={`pluto1`}>Pluto</option>
            </select>
            ...
            <div className="container_down">
                <div>
                    <figure className='text-center' id='exportar'>{/*'exportar es el contenedor donde está el elemento a exportar' */}
                        ...
                        <img id="imagen" src={`./memesImg/${imgmeme}.png`} />{/*si la imagen viene de una API, se configura esto, para que lo reciba */}               
                        <p className='mt-5 h1 text-center estilo estilo2'>{textmemeabajo}</p>
                    </figure>
                </div>
                <div>
                    <button onClick={imgDown}>Descargar</button>
                </div>
            </div>
          </div>       
    );
